...
- (instancetype)initWithMaxDate:(NSDate *)maxDate minDate:(NSDate *)minDate currentDate:(NSDate *)currentDate datePickerMode:(UIDatePickerMode)mode
{
    if (self = [super init]) {
        ...
        //DatePicker属性设置
        _selectDate = currentDate?:[NSDate date];
        _pickerView.datePicker.date = _selectDate;
        _pickerView.datePicker.minimumDate = minDate;
        _pickerView.datePicker.maximumDate = maxDate;
        _pickerView.datePicker.datePickerMode = mode?:UIDatePickerModeDateAndTime;
        
        [self pushDatePicker];//直接显示
    }
    return self;
}
