...
- (void)setMenuTitles:(NSArray *)menuTitles{
    ...
    if (menuTitles.count) {
        ...
        for (int i = 0; i < menuTitles.count; i ++) {
            ...
            //左对齐
            [menuBtn setContentHorizontalAlignment:UIControlContentHorizontalAlignmentLeft];
            [menuBtn setContentEdgeInsets:UIEdgeInsetsMake(0, 10, 0, 0)];
            
            //省略号靠右侧
            menuBtn.titleLabel.lineBreakMode = NSLineBreakByTruncatingTail;
            [menuBtn setTitle:menuTitles[i] forState:UIControlStateNormal];
            [menuBtn setTitleColor:RGB(74, 74, 74) forState:UIControlStateNormal];
...
- (void)menuBtnAction:(UIButton *)button{
    ...
    CGFloat kMaskMargin = self.menuSize.height + 64+44;
//    if (_isCannotScrool) {
//        kMaskMargin = 200 +44 +self.menuSize.height;
//    }
...
- (void)selectedMenuItemWitnMenuIndex:(NSInteger)menuIndex dataModel:(MenuDataModel *)model indexPath:(NSIndexPath *)indexPath{
    ...
    if ([self.delegate respondsToSelector:@selector(MenuDidSelectedItemWithMenuBtnIndex:menuItem: leftMenuItemString:)]) {
        [self.delegate MenuDidSelectedItemWithMenuBtnIndex:indexPath.row menuItem:menuItem leftMenuItemString:@"Just exist singleTable"];
    }
}
