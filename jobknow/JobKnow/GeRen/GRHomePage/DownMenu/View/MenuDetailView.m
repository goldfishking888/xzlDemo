...
static CGFloat const kMenuBookItemVeiwHeiht = 50.0f;
...
- (void)configSingleTableView{
    ...
    tableView.rowHeight = kMenuBookItemVeiwHeiht;
    tableView.bounces = NO;
    [tableView registerClass:[MenueDetailTableViewCell class] forCellReuseIdentifier:@"MenueDetailTableViewCell"];
...
//            GRBookerModel *model_booker = dataArray[i];
//            model.title = model_booker.bookPostName;
            NSDictionary *dic = dataArray[i];
            model.title = [dic valueForKey:@"title"];
            model.nums = [dic valueForKey:@"count"];
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (self.menuType == MenuDropTypeSingleCol) {
//        MenueDetailTableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:@"MenueDetailTableViewCell" forIndexPath:indexPath];
        MenueDetailTableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:@"MenueDetailTableViewCell"];
        if (cell == nil) {
            
            cell = [[MenueDetailTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MenueDetailTableViewCell"];
...
- (void)changeMainViewWithAnimateClickTheSameBtn:(BOOL)isSame{
    if (isSame == NO) {
        ...
        if (self.menuType ==  MenuDropTypeSingleCol) {
            ...
            tableHeight = kMenuBookItemVeiwHeiht * self.dataArray.count;
        } else {
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
     [[NSNotificationCenter defaultCenter] postNotificationName:@"kReSetTableViewContentInsetNSNotification" object:nil];
    [self JKchangeMenuViewBtnsStatus];
    [self JKHandleIsExpendMenuView];
    [self removeFromSuperview];
}
