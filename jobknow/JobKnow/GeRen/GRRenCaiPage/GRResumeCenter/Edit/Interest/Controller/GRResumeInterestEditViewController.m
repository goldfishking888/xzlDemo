//
//  GRResumeInterestEditViewController.m
//  JobKnow
//
...
@interface GRResumeInterestEditViewController ()

@end

@implementation GRResumeInterestEditViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    [self addBackBtnGR];
    [self addTitleLabelGR:@"兴趣爱好"];
...
-(void)initView{
    ...
    placeholderLabel.text = @"请填写";
    if (![_model.content isNullOrEmpty]) {
    ...
    tv_content.text = _model.content;
    ...
    label_count.text = [NSString stringWithFormat:@"%lu/%d",(unsigned long)_model.content.length,MaxCount];
...
{
    if(![_model.content isEqualToString:tv_content.text]){
...
-(void)saveingRequest{
    ...
    [paramDic setValue:tv_content.text forKey:@"interests"];
...
- (void)textViewDidChange:(UITextView *)textView{
    ...
    label_count.text = [NSString stringWithFormat:@"%lu/%d",(unsigned long)textView.text.length,MaxCount];
