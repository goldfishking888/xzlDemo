...
- (void)bindStyleButton:(UIButton *)btn redNum:(NSInteger)redNum status:(BOOL)isSelected {
    ...
    [self.titleButton setTitleColor:isSelected ?  self.themeColor : RGBA(74, 74, 74, 1) forState:UIControlStateNormal];
...
- (void)setUpButtons {
    ...
    for (NSInteger index = 0; index < [self.delegate numberOfItemsInYJSliderView:self]; index ++) {
        ...
        [btn.titleLabel setFont:[UIFont systemFontOfSize:self.fontSize ? self.fontSize : 16.0]];
...
- (void)updateLabelInCellAtIndex:(CGFloat)index nextIndex:(CGFloat)nextIndex {
    ...
    [self changeButtonColor:nextBtn To:self.themeColor WithScale:0];
    [self changeButtonColor:currentBtn To:RGBA(74, 74, 74, 1) WithScale:0];
