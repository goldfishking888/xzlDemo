...
-(void)example01
{
    ...
    [Network getLaunchAdImageDataSuccess:^(NSDictionary * response) {
        ...
        NSString *imgUrl = [NSString stringWithFormat:@"%@%@",KAPPAPI,model.content];
        NSLog(@"imageurl is == %@",imgUrl);
...
//        if([XHLaunchAd checkImageInCacheWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",KAPPAPI,model.content]]])
//        {
//            //设置要添加的自定义视图(可选)
...
-(void)mainAc:(NSDictionary *)dic{
    //广告数据转模型
    LaunchAdModel *model = [[LaunchAdModel alloc] initWithDict:dic];
    //配置广告数据
    XHLaunchImageAdConfiguration *imageAdconfiguration = [XHLaunchImageAdConfiguration new];
    //广告停留时间
    ...
    imageAdconfiguration.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height);
    //广告图片URLString/或本地图片名(.jpg/.gif请带上后缀)
    //        imageAdconfiguration.imageNameOrURLString = [NSString stringWithFormat:@"%@%@",KAPPAPI,model.content];
    imageAdconfiguration.imageNameOrURLString = model.content;
    //缓存机制(仅对网络图片有效)
    //为告展示效果更好,可设置为XHLaunchAdImageCacheInBackground,先缓存,下次显示
    imageAdconfiguration.imageOption = XHLaunchAdImageDefault;
