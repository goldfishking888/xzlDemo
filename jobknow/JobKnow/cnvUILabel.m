...
- (void)cnv_setUIlabelTextColor:(UIColor *) strColor
                andKeyWordColor: (UIColor *) keyColor
          andSecondKeyWordColor:(UIColor *)secondKeyColor
{
    self.stringColor = strColor;
    self.keywordColor = keyColor;
    self.secondKeyWordColor = secondKeyColor;
}
...
- (void)cnv_setUILabelText:(NSString *)string andKeyWord:(NSString *)keyword andSecondKeyWord:(NSString *)secondKeyWord
{
    if (self.text != string) {
        self.text = string;
    }
    [self saveKeywordRangeOfText:keyword andSecondKeyWord:secondKeyWord];
}
- (void) saveKeywordRangeOfText:(NSString *)keyWord andSecondKeyWord:(NSString *)secondKeyWord
{
    if (nil == keyWord) {
        return;
    }
    ...
    if (secondKeyWord!=nil)
    {
        NSRange range1 = [self.text rangeOfString:secondKeyWord];
        NSValue *value1 = [NSValue valueWithRange:range1];
        [list addObject:value1];
    }
}

- (void) cnv_setUILabelText3:(NSString *)string andKeyWord:(NSString *)keyword
{
    if (self.text != string) {
        self.text = string;
    }
    [self saveKeywordRangeOfText3:keyword];
}
...
//保存关键字再Text中的位置信息
- (void) saveKeywordRangeOfText3:(NSString *)keyWord
{
    //有效性校验
    if (nil == keyWord) {
        return;
...
- (NSAttributedString *)illuminatedString:(NSString *)text 
    ...
    {
        NSValue *value1 = [list objectAtIndex:0];
        NSRange range1 = [value1 rangeValue];
        [mutaString addAttribute:(NSString *)(kCTForegroundColorAttributeName) value:(id)self.keywordColor range:range1];
    }
    if (self.secondKeyWordColor!=nil)
    {
        NSValue *value2 = [list objectAtIndex:1];
        NSRange range2 = [value2 rangeValue];
        [mutaString addAttribute:(NSString *)(kCTForegroundColorAttributeName) value:(id)self.secondKeyWordColor range:range2];
    }
