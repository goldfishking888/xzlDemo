...
NSString *const XHQzStateKey = @"XHqiuzhiState";
...
- (void)setAvatarImage:(UIImage *)avatarImage {
    if (avatarImage) {
        [_headView setImage:avatarImage];
//        [_headView setImage:];
//        [_avatarButton setImage:nil forState:UIControlStateNormal];
    }
}

- (void)setAvatarUrlString:(NSString *)avatarUrlString {
    if (avatarUrlString) {
        [_headView setImageURL:[[NSURL alloc] initWithString:avatarUrlString]];
    }else{
//        _headView =  [[EGOImageView alloc] init];
    }
}

//set 建立公开状态
- (void)setJianliState:(NSString *)state{
    [self viewAnimation:_statusBtn frame:CGRectMake(200, 0, 120, 30) time:0.2 alph:1];
    if ([state integerValue]==0) {
        [_statusBtn setImage:[UIImage imageNamed:@"jianliState0"] forState:UIControlStateNormal];
    }else if ([state integerValue]==1){
        [_statusBtn setImage:[UIImage imageNamed:@"jianliState1"] forState:UIControlStateNormal];
    }
...
- (void)setInfo:(NSDictionary *)info {
    ...
    NSString *qiuzhiString = [info valueForKey:XHQzStateKey];
    if (qiuzhiString) {
        NSString*newState=nil;
        switch ([qiuzhiString integerValue]) {
            case 0:
                newState=@"";
                break;
            case 1:
            ...
            case 4:
                ...
                break;
            case 5:
                newState=@"行业了解中暂不考虑换工作";
                break;
            default:
                break;
        }
        self.qzStateLab.text = newState;
    }
...
- (void)_setup {
    ...
    CGFloat avatarButtonHeight = 60;
    self.showUserInfoViewOffsetHeight = CGRectGetHeight(self.frame) - waterDropRefreshHeight / 3 - avatarButtonHeight-45;
    
    _showView = [[UIView alloc] initWithFrame:CGRectMake(0, self.showUserInfoViewOffsetHeight, CGRectGetWidth(self.bounds), waterDropRefreshHeight+45)];
    _showView.backgroundColor = [UIColor clearColor];
    [self addSubview:self.showView];
    
    _headView = [[EGOImageView alloc]initWithFrame:CGRectMake(15, 45, avatarButtonHeight, avatarButtonHeight)];
    _headView.layer.cornerRadius = 30.0;
    _headView.layer.masksToBounds = YES;
//    [_headView setImage:[UIImage imageNamed:@"header_default"]];

    _avatarButton = [UIButton buttonWithType:UIButtonTypeCustom];
    _avatarButton.frame =CGRectMake(15, 45, avatarButtonHeight, avatarButtonHeight);
    [_avatarButton addTarget:self action:@selector(changeHead) forControlEvents:UIControlEventTouchUpInside];
   
    
    _userNameLabel = [[UILabel alloc] initWithFrame:CGRectMake(93,45, 207, 34)];
...
//    _userNameLabel.shadowOffset = CGSizeMake(0, 2);
    ...
    _birthdayLabel = [[UILabel alloc] initWithFrame:CGRectMake(93, 42+45-9, 207, 24)];
...
//    _birthdayLabel.shadowOffset = CGSizeMake(0, 2);
    ...
    //求职状态
    _qzStateLab = [[UILabel alloc]initWithFrame:CGRectMake(93, 42+45+10, 207, 24)];
    _qzStateLab.textColor = [UIColor whiteColor];
    _qzStateLab.backgroundColor = [UIColor clearColor];
    _qzStateLab.font = [UIFont systemFontOfSize:14.0f];
    
    
    //简历公开按钮
    _statusBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    _statusBtn.frame = CGRectMake(200, 0, 120, 30);
    [_statusBtn addTarget:self action:@selector(changeStatus) forControlEvents:UIControlEventTouchUpInside];
//    [_statusBtn setBackgroundColor:[UIColor yellowColor]];
    
    
    [_showView addSubview:_statusBtn];
    [_showView addSubview:self.headView];
    [_showView addSubview:self.avatarButton];
    [_showView addSubview:self.userNameLabel];
    [_showView addSubview:self.birthdayLabel];
    [_showView addSubview:self.qzStateLab];
}
- (void)dealloc {
...
- (void)changeHead
{
    [self.deleat changeHeadViewf];
}
- (void)changeStatus
{
//    NSLog(@"建立是否公开");
    [self viewAnimation:_statusBtn frame:CGRectMake(320, 0, 120, 30) time:0.2 alph:0];
    [self.deleat kaiguan];
}
