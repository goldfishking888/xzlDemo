...
{
    __block BOOL res = NO;
    DBHelper *DB = [DBHelper shareInstance];
...
+ (BOOL)saveOrUpdateObjects:(NSArray *)array{
    ...
    DBHelper *DB = [DBHelper shareInstance];
    // 如果要支持事务
    [DB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        for (DBModel *model in array) {
            if (model.pk <= 0) {//保存
...
- (int)getPkWithParamName:(NSString *)paramName paramValue:(NSString *)paramValue{
    ...
    [DB.dbQueue inDatabase:^(FMDatabase *db) {
        NSString *tableName = NSStringFromClass(self.class);
        NSString *sql = [NSString stringWithFormat:@"SELECT * FROM %@ WHERE %@ = %@",tableName,paramName,paramValue];
        FMResultSet *resultSet = [db executeQuery:sql];
        while ([resultSet next]) {
            DBModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
    ...
    if ([users count] == 0) {
        ...
        DBModel *model =[users firstObject];
        return model.pk;
    }else{
        NSLog(@"存在重复数据~~~");
        DBModel *model =[users firstObject];
