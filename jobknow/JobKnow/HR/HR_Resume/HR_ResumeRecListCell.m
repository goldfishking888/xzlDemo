...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        btn_all = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 40, 80)];
        ...
        btn_all.tag = 21;
        [btn_all addTarget:self action:@selector(chooseResume) forControlEvents:UIControlEventTouchUpInside];
        ...
        logoImage.frame=CGRectMake(Cell_Space_X+Cell_Space, 10, 55, 55);
        [logoImage setTag:2];
        ...
        //标题
        UILabel *titlelabel =[[UILabel alloc] initWithFrame:CGRectMake(Cell_Space_X+Cell_Space*2+logoImage.frame.size.width ,10, 60 , 15)];
        titlelabel.tag=3;
        ...
        //性别和年龄
        UIButton *btn_gender_age = [[UIButton alloc] initWithFrame:CGRectMake(Cell_Space_X+Cell_Space*2+logoImage.frame.size.width+titlelabel.frame.size.width-5 ,13, 50 , 15)];
...
//        [btn_price setTag:9];
        ...
        //工作年限
        UILabel *workDateLabel =[[UILabel alloc] initWithFrame:CGRectMake(Cell_Space_X+Cell_Space*2+logoImage.frame.size.width, 34, 80, 14)];
        workDateLabel.tag=4;
        ...
        //标题
        UILabel *positionlabel =[[UILabel alloc] initWithFrame:CGRectMake(Cell_Space_X+Cell_Space*2+logoImage.frame.size.width ,55, 115 , 15)];
        positionlabel.tag=7;
...
-(void)setModel:(HRReumeModel *)model{
    ...
    [nameLabel setFrame:CGRectMake(Cell_Space_X+Cell_Space*2+logoImage.frame.size.width ,10, 60 , 15)];
    ...
    if (![[NSString stringWithFormat:@"%@",model.age] isEqualToString:@""]) {
        ...
        [btn_gender_age setImageEdgeInsets:UIEdgeInsetsMake(0, 0, 0, 0)];
        //        [btn_gender_age setContentMode:UIViewContentModeBottomLeft];
        //        [btn_gender_age.imageView setContentMode:UIViewContentModeScaleToFill];
    }
...
-(void)clickRsumePrice{
    if ([_delegate respondsToSelector:@selector(addResumePriceFromRec:IndexPath:)]) {
        [_delegate addResumePriceFromRec:_resumeModel IndexPath:_indexPath_fromList];
    }
}

-(void)chooseResume{
    NSIndexPath *path = self.indexPath_fromList;
    [_delegate chooseBtnClickInRow:path];
}
