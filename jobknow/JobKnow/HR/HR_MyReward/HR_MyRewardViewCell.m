//
//  HR_MyRewardViewCell.m
//  JobKnow
//
//  Created by Wangjinyu on 11/08/2015.
...
@implementation HR_MyRewardViewCell
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        participateLabel = [[UILabel alloc] init];
        ...
        [bottomImageView addSubview:participateLabel];
    }
    return self;
}
...
//    NSArray *statusArray = @[@"提现中", @"已发放", @"可提现", @"发放失败，已离职", @"提现成功", @""];
    ...
    upImageView.image = [[UIImage imageNamed:imageArray[[self.dataDic[@"error"] intValue]]] stretchableImageWithLeftCapWidth:20 topCapHeight:20];
    
    frame = CGRectMake(8, 46+topHeight, self.frame.size.width - 16, 81);
    ...
    frame = CGRectMake(215, 52, upImageView.frame.size.width - 235, 17);
    participateLabel.frame = frame;
    if ([[NSString stringWithFormat:@"%@", self.dataDic[@"fromWhere"]] isEqualToString:@"3"]) {
        participateLabel.text = @"参与推荐";
    }else{
        participateLabel.text = @"";
    }
    participateLabel.textColor = RGBA(255, 146, 4, 1);
    participateLabel.textAlignment = NSTextAlignmentRight;
    participateLabel.font = [UIFont systemFontOfSize:12];
}
