...
@implementation HR_JobDetailView
@synthesize delegate;
@synthesize height;
...
//        if ([_positionModel.senior_type isEqualToString:@"1"]) {
...
//        }
        ...
        //职位名称
        _jobName = [[CustomLabel alloc]labelinitWithText2:_positionModel.jobName X:cell_inner_space+40 +2 Y:2];
        _jobName.font = [UIFont fontWithName:@"Arial-BoldMT" size:16];
        ...
        CGSize size = [text sizeWithFont:[UIFont systemFontOfSize:14.2] constrainedToSize:autoSize lineBreakMode:NSLineBreakByWordWrapping];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section==0) {
        ...
        if (indexPath.row ==0) {
            ...
            NSString *salaryStr;
//            if (1) {
//                salaryStr=[NSString stringWithFormat:@"%@：<font color='#f76806'>%@</font>",@"奖金",@"简历推荐奖金"];
//                UILabel *label_q = [[UILabel alloc] initWithFrame:CGRectMake(155, 16, 15, 15)];
...
//            }else{
            NSString * str = [NSString stringWithFormat:@"%@",_positionModel.newfee];
                if ([str hasSuffix:@"以上"]) {
                    salaryStr =[NSString stringWithFormat:@"%@元以上",[_positionModel.newfee substringToIndex:[_positionModel.newfee length]-2]];
                    salaryStr = [NSString stringWithFormat:@"%@：<font color='#f76806'>%@</font>",@"入职奖金",salaryStr];
                }else{
                    salaryStr=[NSString stringWithFormat:@"%@：<font color='#f76806'>%@元</font>",@"入职奖金",_positionModel.newfee];
                }
                
                
                UILabel *label_q = [[UILabel alloc] initWithFrame:CGRectMake(175, 18, 15, 15)];
                [label_q.layer setCornerRadius:label_q.frame.size.width/2];
                [label_q.layer setMasksToBounds:YES];
                [label_q setBackgroundColor:[UIColor colorWithHex:0xf76806 alpha:1]];
                [label_q setText:@"?"];
...
//            if ([[NSString stringWithFormat:@"%@",[[NSUserDefaults standardUserDefaults] valueForKey:@"is_pay_vip"]] isEqualToString:@"1"]) {
//                [jiantou setHidden:YES];
//            }
        }else if (indexPath.row==1) {
            
            //发布日期
            _pubDateLab = [[UILabel alloc] initWithFrame:CGRectMake(cell_inner_space,10, iPhone_width - 165, 30)];
        ...
        {
            //截止日期
            _finishDateLab = [[UILabel alloc] initWithFrame:CGRectMake(cell_inner_space,10, iPhone_width - 160, 30)];
        ...
        {
            //工作地点
            _areaLab = [[UILabel alloc] initWithFrame:CGRectMake(cell_inner_space,10, iPhone_width - 165, 30)];
        ...
        }else if(indexPath.row==4)
        {
            ...
            //薪资待遇
            _salaryLab =  [[RTLabel alloc] initWithFrame:CGRectMake(cell_inner_space,17,iPhone_width - 150,30)];
        ...
        }else if(indexPath.row==5)
        {
            ...
            _jobDescription = [[UILabel alloc] initWithFrame:CGRectMake(cell_inner_space,0, iPhone_width, 30)];
            ...
            _require = [[CustomLabel alloc]labelinitWithText:text X:cell_inner_space Y:30];
            _require.backgroundColor = [UIColor clearColor];
            [cell.contentView addSubview:_jobDescription];
            [_require setBackgroundColor:[UIColor clearColor]];
            ...
            //小灯泡
            UIImageView *imgView = [[UIImageView alloc]initWithFrame:CGRectMake(cell_inner_space,requireHeigh+35, 20, 20)];
...
//        if ([_positionModel.senior_type isEqualToString:@"1"]) { 3.3.1
//            //简历职位
//            [self.delegate showResumeIntroInfo];
//        }else{
//            //显示alert 只要推荐 奖金就有分
            [self.delegate showRuzhiIntroInfo];
//        }
    }
...
//            height+=_requireHeight+50;
            return _requireHeight+76;
        }
    }
