...
#import "OtherLogin.h"
@implementation HRNetWorkConnection
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    if ([self.delegate  respondsToSelector:@selector(receiveDataFinish:Connection:)]) {
        [self.delegate receiveDataFinish:receiveData Connection:connection];
    }
}

- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error
{
    if ([self.delegate respondsToSelector:@selector(receiveDataFail:)]) {
        [self.delegate receiveDataFail:error];
    }
}
...
- (void)sendHRResumeImageToServerWithImage:(UIImage *)image imageName:(NSString *)name resume:(BOOL)resume resumeDic:(HRResumeInfoModel *)resumeModel
{
    ...
    //根据url初始化request
    request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:@"http://timages.hrbanlv.com/pm3/upload"]
    ...
    NSMutableDictionary *params = [NSMutableDictionary dictionaryWithObjectsAndKeys:IMEI,@"userImei",kUserTokenStr,@"userToken",@"2",@"flag",resumeModel.pid,@"pid",nil];
...
- (void)sendHRResumeVoiceToServerWithSound:(NSString *)soundPath soundName:(NSString *)name min:(NSString *)minetus resume:(BOOL)resume resumedic:(HRResumeInfoModel *)resumeModel
{
    ...
    NSMutableDictionary *params = [NSMutableDictionary dictionaryWithObjectsAndKeys:IMEI,@"userImei",kUserTokenStr,@"userToken",resumeModel.pid,@"pid",@"0",@"flag",minetus,@"soundTime", nil];
    if (resume) {
