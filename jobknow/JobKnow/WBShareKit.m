...
- (void)sinaRequestTokenTicket:(OAServiceTicket *)ticket finishedWithData:(NSMutableData *)data {
  ...
  [self performSelector:@selector(doShowSafariWebView:)
             withObject:url];
...
- (void)sendSinaRecordWithStatus:(NSString *)_status lat:(double)_lat lng:(double)_lng delegate:(id)_delegate successSelector:(SEL)_sSel failSelector:(SEL)_eSel
{
	...
	OAMutableURLRequest *request = [self sinaRequestWithURL:url
													dic:[NSMutableDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"经发局"], @"status", nil] 
												 method:@"POST"];
...
#pragma mark - 显示safariView 程序内登录的界面
...
//    [backBtn release];
      ...
      [safariView addSubview:navigationView];
     ...
     // [backBtn setTitle:@"返回" forState:UIControlStateNormal];
      ...
      [backBtn addTarget:self action:@selector(removeSafariview) forControlEvents:UIControlEventTouchUpInside];
      [safariView addSubview:backBtn];
      UIWebView *_curWebView = [[UIWebView alloc]init];
      if (iPhone_5Screen) {
           _curWebView.frame = CGRectMake(0, 44, 320, iPhone_height-44-20);
      }else{
          _curWebView.frame = CGRectMake(0, 44, 320, 416);
      }
  ...
  [window addSubview:safariView];
    if (iPhone_5Screen) {
        safariView.frame = CGRectMake(0, 20, 320, iPhone_height-20);
    }else{
